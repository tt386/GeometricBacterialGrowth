...
def MonoCulture_Activity(t,z,r,b,g1,g2,T):
    ...
    s = sigmoid(X, T, k=10/T)  # smooth transition centered at X=T
    dIdt = s * g2 * (1 - I) + (1 - s) * (-g1 * I)
...
def CoCulture_From_Mono(t,z,kl):#,ST,PT):
    ...
    S_s = sigmoid(SX,ST,10/ST)

    dSIdt =  S_s * Sg2 * (1 - SI) + (1 - S_s) * (-Sg1 * SI)

    Srate = Sr * np.exp(-Sb*SI)
    ...
    P_s = sigmoid(PX,PT,10/PT)

    dPIdt =  P_s * Pg2 * (1 - PI) + (1 - P_s) * (-Pg1 * PI)

    Prate = Pr * np.exp(-Pb*PI)
    ...
    return[dSXdt,dPXdt,dLdt,dSIdt,dPIdt]
##############################################################################
...
def CoCulture_Independently(t,z,
    ...
    S_s = sigmoid(max(SX,PX),ST,10/ST)
    ...
    P_s = sigmoid(max(PX,SX),PT,10/PT)
...
ST=PT = np.log10(10**8)             #Time at which switching happens
kl = np.log10(10**-10)
...
lower = [-1,0,-2,-2,0,6]
upper = [1,np.inf,3,3,10,10]

##############################################
#S Monoculture
params_init = [Sr,Sb,Sg1,Sg2,ST,SIC]

objective = MakeObjective(MonoCulture_Activity,[S_Mono],[S_Mono[0],0])
S_Mono_results = least_squares(objective,params_init,
        ftol=1e-10,f_scale=0.05,loss='soft_l1',bounds=(lower,upper))
...
Fitted_S_Mono = solve_ivp(MonoCulture_Activity,[0,len(S_Mono)],
        (10**Mono_S_Params[-1:],1),args=tuple(Mono_S_Params[:-1]),
        t_eval=np.linspace(0,len(S_Mono)-1,100))
Fitted_P_Mono = solve_ivp(MonoCulture_Activity,[0,len(P_Mono)],
        (10**Mono_P_Params[-1:],1) ,args=tuple(Mono_P_Params[:-1]),
        t_eval=np.linspace(0,len(P_Mono)-1,100))
...
plt.savefig("dIdt Activity_Fitting.png")
plt.close()
...
params_init = [kl,SIC,PIC]
objective = MakeObjective(CoCulture_From_Mono,[S_Co,P_Co],[SIC,PIC,0,1,1])
results = least_squares(objective,params_init,
        ftol=1e-10,f_scale=0.05,loss='soft_l1')

print(10**results.x)
kl,SIC,PIC = 10**results.x

ICs = [10**results.x[1],10**results.x[2],0,1,1]
...
plt.savefig("dIdt Coculture_From_Mono.png")
...
for DATA in Values:
    ...
    upperbounds = [1,3,2,2,10,       1,3,2,2,10,      -9,      np.inf,np.inf]


    objective = MakeObjective(CoCulture_Independently,[S_Co,P_Co],[S_Co[0],P_Co[0],0,1,1])
    ...
    ICs = [10**results.x[-2],10**results.x[-1],0,1,1]
    Fitted = solve_ivp(CoCulture_Independently,[0,len(S_Co)],ICs,args=tuple(results.x[:-2]),
    ...
    plt.savefig("dIdt Coculture_Independently_" +str(DATA) + ".png")
    plt.close()
